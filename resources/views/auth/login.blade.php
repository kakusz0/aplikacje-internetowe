            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" required autofocus autocomplete="email"
                        value="{{ old('email') }}"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400" aria-label="Email" />
                </div>
                <div>
                    <label for="password" class="block mb-1 font-medium text-gray-700">Hasło</label>
                    <input id="password" name="password" type="password" required autocomplete="current-password"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-violet-400"aria-label="Hasło" />
                </div>
                <button type="submit"
                    class="w-full bg-violet-700 text-white font-bold py-2 rounded-lg hover:bg-violet-800 transition"
                    aria-label="Zaloguj się">Zaloguj
                    się</button>
            </form>
